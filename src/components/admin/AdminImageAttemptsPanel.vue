...
<template>
  <section class="min-w-0 rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-sm)] p-4 max-w-[1360px] mx-auto mb-3.5" aria-label="生图监控">
    <div class="flex items-start justify-between gap-2.5 mb-3 max-[680px]:flex-col max-[680px]:items-start">
      ...
      <form class="flex items-center justify-end gap-2 flex-wrap max-[680px]:justify-start max-[680px]:w-full" @submit.prevent="emit('refresh')">
        <select
          :value="statusFilter"
          :disabled="loading"
          class="min-h-9 min-w-[110px] cursor-pointer rounded-md border border-[var(--border)] bg-[var(--input-bg)] text-[var(--text-primary)] px-2 py-1.5 text-sm"
        ...
        <input
          ...
          placeholder="用户 ID"
          :disabled="loading"
          class="min-h-9 w-[150px] min-w-[110px] rounded-md border border-[var(--border)] bg-[var(--input-bg)] text-[var(--text-primary)] px-2 py-1.5 text-sm"
        ...
        <input
          ...
          placeholder="HTTP"
          :disabled="loading"
          class="min-h-9 w-24 rounded-md border border-[var(--border)] bg-[var(--input-bg)] text-[var(--text-primary)] px-2 py-1.5 text-sm"
    ...
    <div class="grid grid-cols-4 gap-2.5 mb-2.5 max-[980px]:grid-cols-2 max-[680px]:grid-cols-1">
      <div class="min-w-0 p-2.5 rounded-md border border-[var(--border)] bg-[var(--surface-soft)]">
    ...
    <div class="w-full overflow-auto rounded-lg border border-[var(--border)]">
      <table class="w-full border-collapse text-[13px] min-w-[1020px]">
        <thead>
          <tr>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">时间</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">用户</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">状态</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">耗时</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">生成 ID</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">错误摘要</th>
            <th class="text-left px-3 py-2 bg-[var(--surface-soft)] text-[var(--text-secondary)] text-xs font-extrabold border-b border-[var(--border)]">详情</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="attempt in attempts" :key="attempt.id" class="border-b border-[var(--border)]">
            ...
            <td colspan="7" class="bg-[var(--surface-soft)] p-3">
              <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <pre class="m-0 p-2 bg-[var(--surface)] border border-[var(--border)] rounded-md text-xs text-red-500 whitespace-pre-wrap break-all">{{ attempt.errorMessage || '-' }}</pre>
                </div>
              </div>
            </td>
          </tr>
