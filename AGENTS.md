...
- `backend/gateway/src/routes/go-sidecar.ts` — Proxies Go-owned routes (`/api/image/*` and `/api/credits*`) to the Go gateway with timeout cleanup and client-disconnect abort handling. Public application and Supabase configuration remain Node-owned routes at `/api/config/app` and `/api/config/supabase`.
