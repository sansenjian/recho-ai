-- Bind the idempotency claim, credit reservation, and initial staging job in
-- one database transaction. If the job insert fails, the credit reservation
-- and transaction row roll back with it. Repeating the call after an unknown
-- network outcome returns the existing staging job instead of reserving twice.
